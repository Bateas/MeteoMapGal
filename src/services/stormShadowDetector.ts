...
export function detectStormShadow(
  ...
  // ── Night/dusk filter: solar detection is meaningless when no sunlight ──
  // Use REAL radiation data: if max radiation across all stations is <50 W/m²,
  // solar analysis is unreliable (night, dusk, or heavy overcast).
  const maxRadiation = Math.max(...snapshots.map(s => s.radiation).filter(r => r >= 0));
  const isNight = maxRadiation < 50;
  if (isNight) {
